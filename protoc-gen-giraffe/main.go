...
func main() {
	...
	if useGogoImport {
		gen.GeneratePlugin(plugin.NewPlugin(plugin.UseGogoImport))
	} else {
		gen.GeneratePlugin(plugin.NewPlugin())
	}

	for i := 0; i < len(gen.Response.File); i++ {
		gen.Response.File[i].Name = proto.String(strings.Replace(*gen.Response.File[i].Name, ".pb.go", ".giraffe.pb.go", -1))
	}
